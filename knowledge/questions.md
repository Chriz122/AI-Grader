...
2.  請撰寫一程式，其功能為：要求由鍵盤輸入橢圓體(如圖所示)的赤道半徑a、b(沿著 X 和y軸)及極半徑c(沿著z軸)，程式可計算橢圓的體積(=$ \frac{4}{3}\pi abc $)及表面積
    表面積近似算法：
    $$ A \approx 4\pi \left(\frac{a^p b^p + a^p c^p + b^p c^p}{3}\right)^{\frac{1}{p}} $$
    其中，$ p \approx 1.0675 $

3.  請撰寫一程式，其功能為：要求由鍵盤輸入”一元二次方程式($ ax^2+bx+c=0 $)”的 a, b, c 三個係數值，則程式可求出其兩個實數解(驗證程式時，輸入的a、b、c值要滿足$ b^2 \geq 4ac $)。(求解公式：
